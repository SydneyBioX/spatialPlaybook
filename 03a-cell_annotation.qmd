...
Identifying cell types is a critical step in spatial analysis, as it shapes how we interpret tissue composition, uncover cell-cell interactions, and understand disease-associated changes at the cellular level. However, cell type annotation is not always straightforward—marker expression often varies continuously rather than forming clear-cut boundaries between populations. Overlapping marker profiles, technical noise, and tissue-specific expression patterns can make annotation uncertain and prone to error, which in turn affects the accuracy of all downstream analyses.

There are two main strategies for assigning cell type labels: clustering and annotation. Clustering is an unsupervised approach that groups cells based on similarities in marker expression. While it can uncover novel or unexpected populations, the resulting clusters must be manually interpreted and labelled using biological expertise and known marker profiles. Annotation, on the other hand, is a supervised method that uses a reference dataset—often derived from well-characterised single-cell data—to directly assign cell type labels to each cell. This can improve consistency and reproducibility but depends heavily on the quality and relevance of the reference used.

Both approaches have advantages and limitations. Clustering may miss subtle populations or be sensitive to parameter choices, while annotation can misclassify cells if the reference does not capture the diversity present in the tissue being analysed. In this playbook, we demonstrate how to use our packages to perform both clustering and annotation, and provide guidance on choosing between these methods based on your data and biological question. Throughout, we highlight common pitfalls and best practices to improve the reliability and interpretability of cell type identification.
...
To demonstrate the functionality of `FuseSOM`, we will use the [Risom 2022](datasets.qmd) dataset, which profiles the spatial landscape of ductal carcinoma in situ (DCIS). We will be using the markers used in the original study to perform clustering.
...
```{r 03a-cluster heatmap, fig.align='center', fig.height=5, fig.width=6, dev='png'}
...
In the next section, we will demonstrate how packages can be used to perform supervised cell annotation, and then discuss how to choose between clustering and annotation approaches.
