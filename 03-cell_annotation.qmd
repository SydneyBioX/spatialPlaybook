...
To demonstrate the use of scClassify, we will use the Lohoff 2022 dataset, which examines spatial relationships during mouse embryo development. It is available through the `MouseGastrulationData` package. We will select samples 21-24 for demonstration purposes.

```{r warning = FALSE, message = FALSE}
mouseSCE = EmbryoAtlasData(samples = c(21, 22, 23, 24))

# remove missing values
mouseSCE = mouseSCE[, !is.na(colData(mouseSCE)$celltype)]
...
```{r}
# log normalisation
norm_assay = normalizeCounts(mouseSCE, transform = "log", BPPARAM = BiocParallel::MulticoreParam(workers = 8))

# store normalised expression data in the "norm" slot
assay(mouseSCE, "norm") = norm_assay
```

The Lohoff 2022 dataset contains 37 unique cell types.

```{r}
mouseSCE$celltype |> unique()
```
...
train_idx = sample(seq_len(ncol(mouseSCE)), size = 0.8 * ncol(mouseSCE))
train_sce = mouseSCE[, train_idx]
test_sce = mouseSCE[, -train_idx]

# access expression data as matrix
train_mat = assay(train_sce, "norm")
test_mat = assay(test_sce, "norm")
```
...
-   `similarity = c("spearman")`: use the Spearman correlation metric to measure correlation between cells. Other options include Pearson's correlation, Kendall's correlation, Jaccard distance, Cosine distance, and weighted rank correlation.

```{r}
scClassify_res = scClassify(exprsMat_train = train_mat,
                            cellTypes_train = train_sce$celltype,
                            exprsMat_test = list(lohoff = test_mat),
                            cellTypes_test = list(lohoff = test_sce$celltype),
                            tree = "HOPACH",
                            algorithm = "WKNN",
                            selectFeatures = c("limma"),
                            similarity = c("spearman"),
                            returnList = FALSE,
                            parallel = TRUE,
